...
class SimpleVariableProcessor extends \yii\base\Component {
    ...
    /* v3 :*/   protected $_pattern = '\{([a-zA-Z0-9\.\_]+)\}';
	...
	protected function replace($matches) {
        ...
        $result = $this->processToken($renderName);
	...
	protected function parseToken($token) {
        ...
        list($fullExpression, $renderName) = $match;
	...
	protected function processToken($name) {
		$renderName = $name;
		$params = null; // $this->paramsStringToParams($paramsString);
        ...
        if (strpos($renderName, '.'))
        {
			...
			if (isset($this->context) && is_object($this->context->{$objectName})) {
				...
				//if ($name == 'parent.testAttribute3') throw new \Exception('t3'.$name);
				...
				if ($this->context->{$objectName}->hasMethod('hasExpressionAttribute') && $this->context->{$objectName}->hasExpressionAttribute($property)) {
					return $this->context->{$objectName}->getParsedAttribute($property, isset($this->variables) ? $this->variables : []);
				} else if (isset($this->context->{$objectName}->{$property})) {
					...
					//if ($name == 'parent.testAttribute3') throw new \Exception('t');
					...
					//throw new \Exception('t2'.$property);
        ...
        if (is_callable([$this->context, 'hasExpressionAttribute']) && is_callable([$this->context, 'getParsedAttribute'])) 
        {
            ...
            {
				return $this->context->getParsedAttribute($renderName, isset($this->variables) ? $this->variables : []);
            }
        }
    ...
    protected function getAnonymousFunction($name, $params = [])
    {
		if (isset($this->variables[$name]) && is_callable($this->variables[$name])) {
			return $this->variables[$name]($params, $this->variables);
		}
    }
}